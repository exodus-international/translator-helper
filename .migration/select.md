...
- src/components/ui/select.tsx — `@radix-ui/react-select` → `@base-ui/react/select`.
  ...
  - Viewport → List (`min-w-(--anchor-width)` popper sizing preserved via Base vars). **Corrected 2026-09-10:** the port also carried over radix's `h-[var(--radix-select-trigger-height)]` as `h-(--anchor-height)`, but Base's `--anchor-height` is the trigger height applied to a List that is not the scroll container radix's Viewport was — it pinned every popup to a single row. Removed; the Popup's `max-h-(--available-height)` already caps and scrolls. Upstream base-vega renders `<Select.List>` with no className at all; ScrollUp/DownButton → ScrollUp/DownArrow (with `top-0 w-full` / `bottom-0 w-full`); Label → GroupLabel; ItemIndicator gets `render={<CheckIcon/>}`; ItemText first with `shrink-0 whitespace-nowrap` per wrapper-shapes anatomy.
