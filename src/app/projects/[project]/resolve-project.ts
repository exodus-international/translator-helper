import { getSourceProjectAction, getSourceProjectBySlugAction } from '@/domain/source-project/source-project.actions';
import { buildProjectPath } from '@/domain/source-project/source-project-url';
import { isUuid } from '@/lib/uuid';
import { notFound, redirect } from 'next/navigation';

/**
 * The {project} segment is a SourceProject.slug. Links shared before that
 ...
 * routes: an admin can rename a slug, and a cached 308 would keep sending
 * people to a path that no longer exists.
 *
...
export async function resolveProject(segment: string, subPath = ''): Promise<SourceProjectDetail> {
  if (isUuid(segment)) {
    ...
    redirect(`${buildProjectPath(project.slug)}${subPath}`);
  }

  const project = await getSourceProjectBySlugAction(segment);
