...
export default function LanguageInstructionsClient({ languages }: LanguageInstructionsClientProps) {
  const initialMap = useMemo(
    () => Object.fromEntries(languages.map((language) => [language.id, language.translationInstructions || ''])),
    [languages],
  );

  const [instructionsByLanguage, setInstructionsByLanguage] = useState<Record<string, string>>(initialMap);
  const [savingLanguageId, setSavingLanguageId] = useState<string | null>(null);
  const [lastSavedMessage, setLastSavedMessage] = useState<Record<string, string>>({});
