...
type TriggerRegistrationAction = 'add' | 'remove';
...
interface StepperContextValue {
  ...
  registerTrigger: (node: HTMLButtonElement | null, action?: TriggerRegistrationAction) => void;
...
function Stepper({
  ...
  const registerTrigger = React.useCallback(
    (node: HTMLButtonElement | null, action: TriggerRegistrationAction = 'add') => {
      if (!node) {
        return;
      }
  ...
  const focusLast = React.useCallback(() => focusTrigger(triggerNodes.length - 1), [focusTrigger, triggerNodes]);
...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  const [currentNode, setCurrentNode] = React.useState<HTMLButtonElement | null>(null);
  ...
  const handleTriggerRef = React.useCallback(
    (node: HTMLButtonElement | null) => {
      if (node) {
        ...
        registerTrigger(currentNode, 'remove');
      }
      setCurrentNode(node);
    },
    [currentNode, registerTrigger],
  );

  // Find our index among triggers for navigation
  const myIdx = React.useMemo(
    () => (currentNode ? triggerNodes.findIndex((n: HTMLButtonElement) => n === currentNode) : -1),
    [triggerNodes, currentNode],
  );
...
interface StepperStatusIndicatorProps extends React.ComponentProps<typeof StepperIndicator> {
  status: DocumentStatus | DocumentStatusKey | null | undefined;
  iconClassName?: string;
}

function StepperStatusIndicator({ status, className, iconClassName, children, ...props }: StepperStatusIndicatorProps) {
  const { state } = useStepItem();
  const statusConfig = getDocumentStatusConfig(status ?? null);
  const StatusIcon = statusConfig.icon;

  // Use darker colors for completed steps, lighter for inactive/todo steps
  ...
  return (
    <StepperIndicator className={cn(indicatorClass, className)} {...props}>
      {children ?? <StatusIcon className={cn('h-3.5 w-3.5', iconColorClass, iconClassName)} />}
    </StepperIndicator>
  );
}
