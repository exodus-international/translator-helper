  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        className={cn(
          'bg-background fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border p-6 shadow-lg transition-[opacity,scale] duration-200 data-starting-style:scale-95 data-starting-style:opacity-0 data-ending-style:scale-95 data-ending-style:opacity-0 sm:max-w-lg',
          className,
        )}
        {...props}
      />
...
function AlertDialogAction({ className, ...props }: React.ComponentProps<typeof AlertDialogPrimitive.Close>) {
  return (
    // Base UI has no Action part; Close with action semantics is the
    // shadcn-base-idiomatic replacement (clicking still dismisses the dialog).
